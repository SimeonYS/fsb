...
class FsbPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('fsb.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `fsb`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM fsb WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO fsb (date, title, link, content)"
